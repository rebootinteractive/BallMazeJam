...
export class MainMenu {
  ...
  private render() {
    ...
    title.textContent = 'BallMazeJam';
    this.root.appendChild(title);

    const sub = document.createElement('div');
    sub.className = 'menu-sub';
    sub.textContent = 'Swipe a ball to slide it. Pair same colors to clear them.';
  ...
  private renderCard(level: LevelData, isCustom: boolean): HTMLDivElement {
    ...
    meta.className = 'meta';
    meta.textContent = `${level.cols}×${level.rows} · ${level.balls.length} ball${
      level.balls.length === 1 ? '' : 's'
    } · ${level.timeSeconds}s`;
